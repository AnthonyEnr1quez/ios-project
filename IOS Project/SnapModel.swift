...
struct SnapModel {
    ...
    init() {
        ...
        //        let steps2 = "Ask the user for n Draw a square \nMove to a random location \nRepeat steps 2-3 n times
        //        "
        ...
        let prob1 = Problem(name: "Draw a Square", algorithm: "Draw a 100 x 100 square", steps: steps1)
        
        let prob2 = Problem(name: "It's Raining Squares!", algorithm: "Draw n 20x20 squares (where n is supplied by the user) in random locations about the screen (like raindrops, but square)", steps: steps2)
        let prob3 = Problem(name: "It's Raining Red and Green Square!", algorithm: "Modify your square-raining program \n", steps: steps3)
        let prob4 = Problem(name: "Bouncing Forever", algorithm: "Starting at the center, heading east, make a sprite move across the screen with sound, bouncing off the walls (literally)", steps: steps4)
        let prob5 = Problem(name: "Worst Guessing Game Ever", algorithm: "Guess a number that the computer is thinking of, between 1-10", steps: steps5)
        let prob6 = Problem(name: "Einstein Walks Into a Bar ...", algorithm: "A random walk is a path that consists of a random set of steps \nExamples: molecules moving through a liquid; path of a foraging animal; stock price; wave height; a drunkard; financial status of a gambler \nApplications in ecology, psychology, physics, computer science, chemistry, biology and economics  ", steps: steps6)
        let prob7 = Problem(name: "Polygons-Gateway to Pinwheels", algorithm: "Draw an n-gon, an n-sided figure", steps: steps7)
    ...
    mutating func setSelectedRowIndex(_ index: Int)  {
        selectedRow = index
    }
